...
## SPM-6 실행 계획 (2026-05-03 기준)

### 1) 현재 구현 현황 요약

- 완료/진행 기반:
  - 레이아웃 골격(헤더/모바일 하단 네비/데스크톱 사이드네비) 반영
  - 인증 기본 구조(`auth/me`, `auth/session`) 및 클럽룸 권한 가드 반영
  - 관리자 경기 API(`admin/matches`)와 관리자 경기 화면 기본 반영
  - 기본 라우트 골격(`dashboard`, `evaluation`, `ranking`, `team-builder`, `admin`) 생성
  - 핵심 모델(User/ClubRoom/Match/Evaluation/BGMTrack) 생성
- 미완/후속 중심:
  - 관리자 멤버 승인/역할 관리 화면 및 API 확장
  - 평가 플로우(결장 선택 → 점수 입력 → MVP 제출)의 완성도 보강
  - 결과 집계/대시보드 시각화/팀 빌더 알고리즘 고도화
  - BGM/YouTube/PWA/배포 최적화 후반 구간

### 2) SPM-6 우선순위 백로그

1. 관리자 멤버 관리(`admin/members`) 구현
2. 평가 입력 UI 고도화(모바일 1열, 데스크톱 2열 + 결장 처리)
3. 평가 제출/중복방지/자동마감 집계 정확성 보강
4. 순위/대시보드 API 응답 스키마 확정 및 차트 렌더 안정화
5. 팀 구성 알고리즘 유닛 테스트 세트(6/8/10명 케이스) 작성

### 3) 완료 기준 (SPM-6)

- 기능 기준:
  - 관리자 멤버 승인/거절/권한 변경이 UI+API에서 일관 동작
  - 평가 플로우 제출 시 중복 제출 방지와 자동 마감 집계가 재현 가능
  - 순위/대시보드/팀구성 핵심 화면이 모바일·데스크톱에서 깨지지 않음
- 품질 기준:
  - `npm run test:run` 통과
  - `npx tsc --noEmit` 통과
  - 변경 영역 테스트 파일 동반 작성
...
## SPM-7 실행 계획 (2026-05-03 기준)

### 1) 현재 구현 현황 업데이트

- SPM-6에서 완료된 핵심:
  - 관리자 멤버 관리 API/화면 기본 구현 완료 (`admin/members`)
  - 평가 흐름을 포지션 제출 선행 구조로 전환 완료
  - 포지션 전원 제출 전 평가 제출 차단, 제출 포지션 기반 평가표 구성 완료
  - 전원 평가 완료 시 자동 마감/결과 집계/결과 조회 API 연결 완료
  - 공용 알림함 모델/API/UI 및 평가 단계 알림 발송(요청/시작/완료) 연동 완료
- SPM-7에서 보강할 핵심:
  - 알림 실시간 반영(폴링 → 소켓 이벤트 기반) 보강
  - 순위/대시보드 API 실데이터 연동
  - 팀 구성 알고리즘 검증(유닛 테스트) 고도화

### 2) SPM-7 우선순위 백로그

1. 알림 실시간화: `notification-created` 이벤트 설계/연동
2. 순위 API 구현: `GET /api/rankings` (overall + metric별)
3. 대시보드 API 구현: `GET /api/users/[id]/dashboard`
4. 팀빌더 알고리즘 유닛 테스트: 6/8/10명 케이스 + edge case
5. 평가 결과 UX 보강: 내 점수/상위권 비교 시각 요소 정교화

### 3) 완료 기준 (SPM-7)

- 기능 기준:
  - 알림함이 신규 알림을 실시간(또는 준실시간)으로 반영
  - 순위/대시보드가 목데이터 없이 DB 집계 기반으로 동작
  - 팀빌더 알고리즘 테스트가 최소 핵심 케이스를 커버
- 품질 기준:
  - `npm run test:run` 통과
  - `npx tsc --noEmit` 통과
  - 신규 API는 `src/app/api/MAP.md`와 도메인 문서에 동기 반영
